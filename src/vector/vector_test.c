...
int main() {
    ...
    struct socket_struct *ss0 = (struct socket_struct *)malloc(sizeof(struct socket_struct *));
    ss0->socket = 10;
    struct socket_struct *ss1 = (struct socket_struct *)malloc(sizeof(struct socket_struct *));
    ss1->socket = 20;
	...
	for (int i = 0; i < 20; i++) {
		printf("%i\n", vector_get(vector->socket->socket, i));
	}
